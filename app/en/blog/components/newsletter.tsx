  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatus('error')
        setErrorMessage(result.error || t('blogNewsletter.errorFailed'))
      }
    } catch (error) {
      setStatus('error')
      setErrorMessage(t('blogNewsletter.errorUnexpected'))
    ...
    <section className="py-20 border-t border-gray-900">
      <div className="container mx-auto px-6 md:px-12">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-6">{t('blogNewsletter.title')}</h2>
          <p className="text-gray-400 mb-8">
            {t('blogNewsletter.subtitle')}
          </p>
          ...
          <form onSubmit={handleSubmit} className="relative">
            <div className="flex flex-col sm:flex-row gap-4">
              <input
                ...
                placeholder={t('blogNewsletter.emailPlaceholder')}
                required
              ...
              >
                {status === 'submitting' ? (
                  ...
                  <>{t('blogNewsletter.buttonSubscribe')} <ArrowRight size={18} className="ml-2" /></>
                )}
              </button>
            </div>
            ...
            <AnimatePresence>
              {status === 'success' && (
                ...
                >
                  {t('blogNewsletter.successMessage')}
                </motion.div>
              )}
              {status === 'error' && (
                <motion.div
